...
id_sources=(
  ...
  programs/ppv_governance/src/lib.rs
)
...
for program in ppv_core ppv_commerce ppv_governance; do
  cp "target/idl/${program}.json" "${workdir}/${program}.json"
done

# The second build is incremental; comparing generated IDLs catches
...
# Assert the four identity surfaces agree before anything is deployed: keypair,
# declare_id!, Anchor.toml and generated IDL.
for program in ppv_core ppv_commerce ppv_governance; do
