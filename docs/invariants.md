...
| 12l | A proof-backed payout is allowed only while the cited evidence's `ppv_core::ProofRecord` is `Active`. | `require_cited_proof` loads the core record — owner, discriminator, recorded address, derived address, authority — and `require_live_core_commitment` refuses a revoked one, before any custody moves. Both settlement paths call the same function. | `a_revoked_core_commitment_cannot_back_a_payout`, `another_proofs_core_record_cannot_stand_in`, "reproduces the finding: a revoked core commitment cannot back a payout", "refuses a tranche backed by a revoked commitment…" |
| 12m | Refusing a citation never refuses the payment. | Citing evidence is optional in both settlement paths, so a revocation removes a justification and not a payout; only the submitter can revoke, so neither party can hold custody hostage. | "leaves the payment reachable, so revocation cannot strand custody", "refuses a citation whose core record was revoked by its own author only" |
