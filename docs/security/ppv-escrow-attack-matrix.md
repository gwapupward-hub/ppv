...
| X-8 | Settle against evidence whose `ppv_core` commitment was revoked after approval | PPV-X4 | `require_live_core_commitment` refuses a status other than `Active`, before any custody moves | `proof.rs::a_revoked_core_commitment_cannot_back_a_payout`, `escrow.ts` "core proof revocation at settlement (RR13-001)" | MODEL + LOCAL-VALIDATOR | pass |
| X-9 | Substitute a different `ppv_core::ProofRecord` for the cited evidence's | PPV-X4 | Owner, discriminator, the recorded address and the `(submitter, agreement, index)` derivation must all agree | `proof.rs::another_proofs_core_record_cannot_stand_in`, `escrow.ts` "refuses every core record but this evidence's own" | MODEL + LOCAL-VALIDATOR | pass |
| X-10 | Cite evidence while withholding its core record, or pass a core record nobody cites | PPV-X4 | `require_cited_proof` requires them both-or-neither | `escrow.ts` "requires the core record with a citation, and refuses one without" | LOCAL-VALIDATOR | pass |
| X-11 | Revoke the counterparty's evidence to spoil a citation | PPV-X4 | `ppv_core::revoke_proof` is authority-only, and the payout does not depend on the citation | `escrow.ts` "refuses a citation whose core record was revoked by its own author only", "leaves the payment reachable, so revocation cannot strand custody" | LOCAL-VALIDATOR | pass |
